...
2. $64 \times 64$矩阵：对于该大小的矩阵，缓存只能完整的存储该矩阵的4行，因此不能用$8 \times 8$ 分块来转置，会造成冲突，因此采用$4 \times 4$的分块大小来进行转置，但是会使得缓存的大小不能得到充分的利用。因此我们采用改进的$8 \times 8$ 分块来进行转置。
